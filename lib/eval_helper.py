...
def get_eval(data_dict, config, answer_vocab=None, use_reference=False, use_lang_classifier=False, use_oracle=False, use_cat_rand=False, use_best=False, post_processing=None):
    ...
    if 'answer_scores' not in data_dict: # if using no_answer
        ...
        data_dict["pred_answer_scores"] = data_dict['answer_scores']
        
    if 'answer_scores_scene' not in data_dict: # if no 3d scene prediction
        data_dict["answer_acc_at1_scene"] = torch.zeros(1)[0].cuda()
        data_dict["answer_acc_at10_scene"] = torch.zeros(1)[0].cuda()
    else:
        # answer
        # data_dict['answer_scores']: batch_size, num_answers
        # data_dict["answer_cats"]: batch_, num_answers
        # pred_answers_at1: batch_size
        num_classes = data_dict['answer_scores_scene'].shape[1]
        pred_answers_at1_scene = torch.argmax(data_dict['answer_scores_scene'], 1)
        # pred_answers: batch_size, num_answers
        data_dict["answer_acc_at1_scene"] = (F.one_hot(pred_answers_at1_scene, num_classes=num_classes).float() 
                                        * data_dict['answer_cats']).max(dim=1)[0].mean()
        topk = 10
        pred_answers_at10_scene = data_dict['answer_scores_scene'].topk(topk, dim=1)[1]
        data_dict["answer_acc_at10_scene"] = (F.one_hot(pred_answers_at10_scene, num_classes=num_classes).sum(dim=1).float()
                                        * data_dict['answer_cats']).max(dim=1)[0].mean()
        
        data_dict["pred_answer_scores_scene"] = data_dict['answer_scores_scene']
    
    if 'answer_scores_2d' not in data_dict: # if no 2d prediction
        data_dict["answer_acc_at1_2d"] = torch.zeros(1)[0].cuda()
        data_dict["answer_acc_at10_2d"] = torch.zeros(1)[0].cuda()
    else:
        # answer
        # data_dict['answer_scores']: batch_size, num_answers
        # data_dict["answer_cats"]: batch_, num_answers
        # pred_answers_at1: batch_size
        num_classes = data_dict['answer_scores_2d'].shape[1]
        pred_answers_at1_2d = torch.argmax(data_dict['answer_scores_2d'], 1)
        # pred_answers: batch_size, num_answers
        data_dict["answer_acc_at1_2d"] = (F.one_hot(pred_answers_at1_2d, num_classes=num_classes).float() 
                                        * data_dict['answer_cats']).max(dim=1)[0].mean()
        topk = 10
        pred_answers_at10_2d = data_dict['answer_scores_2d'].topk(topk, dim=1)[1]
        data_dict["answer_acc_at10_2d"] = (F.one_hot(pred_answers_at10_2d, num_classes=num_classes).sum(dim=1).float()
                                        * data_dict['answer_cats']).max(dim=1)[0].mean()
        
        data_dict["pred_answer_scores_2d"] = data_dict['answer_scores_2d']
        
    # how much does 3d improve than 2d
    if 'answer_scores_2d' in data_dict and 'answer_scores_scene' in data_dict:
        # diff_pred = (pred_answers_at1_scene - pred_answers_at1_2d) != 0
        # in those cases where 2d and 3d disagree, how often does 3d get it right
        correct_3d = (F.one_hot(pred_answers_at1_scene, num_classes=num_classes).float()
                                        * data_dict['answer_cats']).max(dim=1)[0].bool() # [batch_size], bool mask
        correct_2d = (F.one_hot(pred_answers_at1_2d, num_classes=num_classes).float()
                                        * data_dict['answer_cats']).max(dim=1)[0].bool()
        correct_3d_over_2d = correct_3d & ~correct_2d
        data_dict["answer_acc_at1_3d_over_2d"] = correct_3d_over_2d.float().mean()
        # in those cases where 2d and 3d disagree, how often does 2d get it right
        correct_2d_over_3d = ~correct_3d & correct_2d
        data_dict["answer_acc_at1_2d_over_3d"] = correct_2d_over_3d.float().mean()
        
    if 'answer_scores_2d3d' not in data_dict: # if no 2d3d prediction
        data_dict["answer_acc_at1_2d3d"] = torch.zeros(1)[0].cuda()
        data_dict["answer_acc_at10_2d3d"] = torch.zeros(1)[0].cuda()
    else:
        # answer
        # data_dict['answer_scores']: batch_size, num_answers
        # data_dict["answer_cats"]: batch_, num_answers
        # pred_answers_at1: batch_size
        num_classes = data_dict['answer_scores_2d3d'].shape[1]
        pred_answers_at1_2d3d = torch.argmax(data_dict['answer_scores_2d3d'], 1)
        # pred_answers: batch_size, num_answers
        data_dict["answer_acc_at1_2d3d"] = (F.one_hot(pred_answers_at1_2d3d, num_classes=num_classes).float() 
                                        * data_dict['answer_cats']).max(dim=1)[0].mean()
        topk = 10
        pred_answers_at10_2d3d = data_dict['answer_scores_2d3d'].topk(topk, dim=1)[1]
        data_dict["answer_acc_at10_2d3d"] = (F.one_hot(pred_answers_at10_2d3d, num_classes=num_classes).sum(dim=1).float()
                                        * data_dict['answer_cats']).max(dim=1)[0].mean()
    ...
    if 'answer_scores' in data_dict:
        ...
        data_dict["pred_answers"] = pred_answers_at1
