...
class ProjectionHelper():
    def __init__(self, intrinsic, depth_min, depth_max, image_dims, accuracy, cuda=True, device=None):
        ...
        if device is None:
            ...
            self.device = torch.device(f"cuda:{torch.distributed.get_rank()}") if self.cuda else torch.device("cpu")
        else:
            self.device = device
    ...
    def _compute_corner_points(self):
        if self.cuda:
            corner_points = torch.ones(8, 4).to(self.device)
        else:
    ...
    def points_in_frustum(self, corner_coords, normals, new_pts, return_mask=False):
        ...
        point_to_plane1 = (new_pts.to(self.device) - corner_coords[2][:3].view(-1))
        point_to_plane2 = (new_pts.to(self.device) - corner_coords[4][:3].view(-1))
    ...
    def compute_projection(self, points, depth, camera_to_world):
        ...
        ind_points = torch.arange(0, num_points, out=torch.LongTensor()).to(self.device)
        ...
        mask_frustum_bounds = self.points_in_frustum(corner_coords, normals, points, return_mask=True).to(self.device)
        ...
        camera = torch.mm(world_to_camera, coords) # [4, num_points], projected points in camera coordinates
        ...
        depth_vals = torch.index_select(depth.view(-1), 0, valid_image_ind.to(self.device))
