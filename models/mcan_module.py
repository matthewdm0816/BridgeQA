...
class AttFlat(nn.Module):
    ...
    def forward(self, x, x_mask, att_pdrop=0, att_drop_topk=100):
        att = self.mlp(x) # [B, N_token, n_glimpse]
        ...
        # mask off top-K attentive scores, with prob of att_pdrop
        # code idea from 
        # https://github.com/MuchHair/HQM/blob/1263b24c0d50002a0f55fcb8adce8e531fb19311/models/hard_mask_att.py#L250
        if att_pdrop > 0:
            # scores ~ [...(B, N_heads), N_q, N_k]
            print(f"att_pdrop:{att_pdrop}, att_drop_topk:{att_drop_topk}, {att.shape}")
            bsz, ntoken, ng = att.shape
            _, ind = att.topk(k=att_drop_topk, dim=1)
            att_mask = torch.zeros_like(att).long()
            att_mask[
                torch.arange(bsz, device=att.device).view(-1,1,1),
                ind,
                torch.arange(ng, device=att.device).view(1,1,-1)
            ] = 1
            att_mask = att_mask.transpose(-1, -2)
            att_mask[
                ...,
                torch.rand(ntoken, device=att.device).topk(int(ntoken*(1-att_pdrop)))[1]
            ] = 0
            att_mask = att_mask.transpose(-1, -2)
            att.masked_fill_(att_mask.bool(), float('-inf'))
...
class MHAtt(nn.Module):
    ...
    def forward(self, v, k, q, mask, att_pdrop=0, att_drop_topk=100):
        n_batches = q.size(0)
        ...
        atted = self.att(v, k, q, mask, att_pdrop, att_drop_topk)
    ...
    def att(self, value, key, query, mask, att_pdrop=0, att_drop_topk=100):
        d_k = query.size(-1)
        ...
        # if att_pdrop > 0:
        #     # scores ~ [...(B, N_heads), N_q, N_k]
        #     print(f"att_pdrop:{att_pdrop}, att_drop_topk:{att_drop_topk}, {scores.shape}")
        #     bsz, nheads, nq, nk = scores.shape
        #     _, ind = scores.topk(k=att_drop_topk, dim=-1)
        #     scores_mask = torch.zeros_like(scores)
        #     scores_mask[
        #         torch.arange(bsz, device=scores.device).view(-1,1,1,1),
        #         torch.arange(nheads, device=scores.device).view(1,-1,1,1),
        #         torch.arange(nq, device=scores.device).view(1,1,-1,1),
        #         ind,
        #     ] = 1
        #     scores_mask[...,torch.rand(nk, device=scores.device).topk(int(nk*(1-att_pdrop)))[1]] = 0
...
class SGA(nn.Module):
    ...
    def forward(self, x, y, x_mask, y_mask, att_pdrop, att_drop_topk):
        # self-att
        x = self.norm1(x + self.dropout1(
        ...
        # cross-att
        x = self.norm2(x + self.dropout2(
            self.mhatt2(y, y, x, y_mask, att_pdrop, att_drop_topk)
        ))
...
class MCAN_ED(nn.Module):
    ...
    def forward(self, x, y, x_mask, y_mask, att_pdrop: float=0, att_drop_topk: int=100):
        ...
        for dec in self.dec_list:
            y = dec(y, x, y_mask, x_mask, att_pdrop, att_drop_topk)
