    ...
    def __init__(self, num_object_class, use_lang_classifier=True, use_bidir=False, num_layers=1,
        ...
        if self.use_bert_model:
            ...
            if "t5" in bert_model_name:
                self.bert_model = T5EncoderModel.from_pretrained(bert_model_name)
            else:
...
class CLIPLangModule(nn.Module):
    def __init__(self, clip_model_name, clip_ckpt_name, output_size) -> None:
        super().__init__()
        import open_clip
        logger.info(f"Loading CLIP Model {clip_model_name}-{clip_ckpt_name}...")
        self.model, _, _ = open_clip.create_model_and_transforms(clip_model_name, pretrained=clip_ckpt_name)
        self.tokenizer = open_clip.get_tokenizer(clip_model_name)
        self.lang_linear = nn.Linear(self.model.text_projection.data.shape[1], output_size)

    def forward(self, data_dict):
        device = next(self.model.parameters()).device
        text_input = self.tokenizer(data_dict["question"]).to(device)
        lang_feat = self.model.encode_text_seqfeature(text_input)
        lang_feat = self.lang_linear(lang_feat)
        data_dict["lang_out"] = lang_feat # [N, L, D]
        # FIXME: for CLIP, we don't have lang_mask but still [PAD] token
        data_dict["lang_mask"] = torch.zeros(data_dict["lang_out"].shape[:-1], device=device).bool() # [N, L]
        assert len(data_dict["lang_mask"].shape) == 2
        return data_dict
