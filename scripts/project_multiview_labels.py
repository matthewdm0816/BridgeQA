    ...
    for scene_id in tqdm(scene_list):
        ...
        for i, projection in enumerate(projections):
            ...
            if args.gt:
                ...
                image = load_image_unnorm(SCANNET_FRAME_PATH.format(scene_id, "color", "{}.jpg".format(frame_id)), [41, 32])
                # feat = (to_tensor(image).mean(0) * 255.0).long() # grayscale
                feat = (to_tensor(image) * 255.0).long()[-1] # rgb -> b
            else:
            ...
            proj_label = Projection.apply(feat, projection_3d, projection_2d, scene.shape[0]).transpose(1, 0)
