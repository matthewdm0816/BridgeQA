...
def resize_crop_image(image, new_image_dims):
    ...
    image = transforms.Resize([new_image_dims[1], resize_width], interpolation=Image.NEAREST)(image)
...
def load_image_unnorm(file, image_dims):
    image = imread(file)
    # preprocess
    image = resize_crop_image(image, image_dims)
    if len(image.shape) == 3: # color image
...
def visualize(coords, labels, color=False):
    ...
    output_pc = PlyData([output_pc], text=True)
...
if __name__ == "__main__":
    ...
    for scene_id in tqdm(scene_list):
        ...
        print("scene_id: {}, num_frames: {}".format(scene_id, len(frame_list)))
        scene_images = np.zeros((len(frame_list), 3, 256, 328))
        scene_depths = np.zeros((len(frame_list), 256, 328))
        ...
        for i, frame_id in enumerate(frame_list):
            ...
            scene_depths[i] = load_depth(SCANNET_FRAME_PATH.format(scene_id, "depth", "{}.png".format(frame_id)), [328, 256])
        ...
        for i, projection in enumerate(projections):
            ...
            if args.gt:
                ...
                image = load_image_unnorm(SCANNET_FRAME_PATH.format(scene_id, "color", "{}.jpg".format(frame_id)), [328, 256])
                # feat = (to_tensor(image).mean(0) * 255.0).long() # grayscale
                feat = (to_tensor(image).mean(0) * 255.0).long() # rgb -> b
            else:
            ...
            proj_label = proj_label.long() # NOTE: HACK
            ...
            if args.maxpool:
                ...
                if i == 0:
                    ...
                    # if args.color:
                    #     mask = (labels == 0).sum(1) == 128
                    #     labels[mask] = proj_label[mask]
                    # else:
        ...
        if args.maxpool:
            if args.color:
                # labels ~ [num_points, num_frames], 0 is background
                # average except background
                labels = labels.cpu().float().numpy()
                labels[labels == 0] = np.nan
                labels = np.nanmean(labels, 1)
        ...
        # output
        vertex = visualize(scene, labels.long().squeeze(1).cpu().numpy(), color=args.color)
